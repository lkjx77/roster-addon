...
function (_React$PureComponent) {
  (0, _inheritsLoose2.default)(WeekWrapper, _React$PureComponent);
  ...
  _proto.update = function update(event, start, end) {
    var segment = (0, _eventLevels.eventSegments)((0, _extends2.default)({}, event, {
      ...
      start: start,
      __isPreview: true
    }), this.props.slotMetrics.range, _common.dragAccessors); // console.log(`event segments: ${JSON.stringify(segment)}`)
  ...
  _proto.handleResize = function handleResize(point, node) {
    ...
    var rowBox = (0, _Selection.getBoundsForNode)(node);
    var cursorInRow = (0, _selection.pointInBox)(rowBox, point); // console.log(`right resize point : ${JSON.stringify(point)}`)
    ...
    if (direction === 'RIGHT') {
      ...
      if (cursorInRow) {
        if (metrics.last < start) return this.reset(); // const cursorInRowSlot = getSlotAtX(
        //   rowBox,
        //   point.x,
        //   false,
        //   metrics.slots
        ...
        end = _dates.default.add(metrics.getDateForSlot((0, _selection.getSlotAtX)(rowBox, point.x, false, metrics.slots)), 1, 'day'); // console.log(`cursorInRowSlot: ${cursorInRowSlot}, end: ${end}`)
      } else if (_dates.default.inRange(start, metrics.first, metrics.last) && rowBox.right < point.x && rowBox.top < point.y && rowBox.bottom > point.y) {
      ...
      } else if (_dates.default.inRange(start, metrics.first, metrics.last) && rowBox.right > point.x && rowBox.top > point.y) {
        this.setState({
          segment: null
        });
        return;
      ...
      } else if (_dates.default.inRange(end, metrics.first, metrics.last) && rowBox.left > point.x && rowBox.top < point.y && rowBox.bottom > point.y) {
  ...
  _proto.render = function render() {
    ...
    var segment = this.state.segment; // console.log(`draw segment: ${JSON.stringify(segment)}`)
    ...
    return _react.default.createElement("div", {
      ...
      className: "segment begin"
    }), segment && _react.default.createElement(_EventRow.default, (0, _extends2.default)({}, this.props, {
      selected: null,
