...
var _TimeGridEventRoster = _interopRequireDefault(require("./TimeGridEventRoster"));

var DayColumnRoster =
/*#__PURE__*/
function (_React$Component) {
  (0, _inheritsLoose2.default)(DayColumnRoster, _React$Component);

  function DayColumnRoster() {
    var _this;
    ...
    _this.renderEvents = function () {
      ...
      return styledEvents.map(function (_ref, idx) {
        ...
        return _react.default.createElement(_TimeGridEventRoster.default, (_React$createElement = {
          style: style,
          event: event,
          label: label,
          key: 'evt_' + idx,
  ...
  var _proto = DayColumnRoster.prototype;
  ...
  _proto.render = function render() {
    ...
    events.map(function (event) {
      var evtStart = _dates.default.max(event.start, min);

      var evtEnd = _dates.default.min(event.end, max); // if (evtEnd == dates.subtract(max, 59, "seconds")) {
      //   evtEnd = dates.add(max, 1, "seconds")
      // }


      var equalTime = _dates.default.hours(evtEnd) === _dates.default.hours(max) && _dates.default.minutes(evtEnd) === _dates.default.minutes(max);

      if (equalTime) {
        evtEnd = max;
      }

      hours = hours + _dates.default.diff(evtStart, evtEnd, 'seconds') / 3600;
    });
    var customStyle = 'flex-basis: 14.2857%; max-width: 14.2857%;';
    ...
    }, "HeadCount: (", events.length, ") "), _react.default.createElement("div", {
      className: "summary-content"
    }, "Hours: ", hours.toFixed(2)), isNow && _react.default.createElement("div", {
      ref: "timeIndicator",
      className: ""
    }));
  };

  return DayColumnRoster;
}(_react.default.Component);

DayColumnRoster.propTypes = {
...
DayColumnRoster.defaultProps = {
  dragThroughEvents: true,
  timeslots: 2
};
var _default = DayColumnRoster;
